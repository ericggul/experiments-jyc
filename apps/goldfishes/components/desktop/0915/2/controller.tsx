...
export default function DesktopController() {
  ...
  useEffect(() => {
    ...
    async function poll() {
      try {
        ...
        const response = await fetch('/api/desktop/0915/2', { cache: 'no-store', signal: abort.signal });
  ...
  async function act(action: 'start' | 'stop') {
    ...
    const snapshot = { ...draft, apps: [...draft.apps], displayWidth: Math.max(640, Math.min(7680, window.screen.availWidth)), displayHeight: Math.max(480, Math.min(4320, window.screen.availHeight)) };
    try {
      const response = await fetch('/api/desktop/0915/2', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action, settings: snapshot }) });
      const next = await response.json();
  ...
  return <main className={styles.page}>
    <header className={styles.header}><a href="/desktop/0915">← Desktop / 0915</a><span>2 · moving windows</span></header>
