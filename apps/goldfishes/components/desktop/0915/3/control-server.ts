...
type State = { child: ChildProcess | null; message: string; timeout?: ReturnType<typeof setTimeout>; settings?: Settings; progress?: number; errors?: { id: number; text: string }[]; lastAction?: string; elapsedMs?: number; targetMs?: number; ownership4?: { browserIdentity: string; pages: { windowId: number; tabId: number; slot: number }[] }; pageCount?: number; windowCount?: number; title?: string; kind?: string };
...
function reply(status = 200) {
  return Response.json({ enabled: enabled(), running: !!state.child, message: state.message, settings: state.settings, progress: state.progress ?? 0, errors: state.errors ?? [], lastAction: state.lastAction, elapsedMs: state.elapsedMs, targetMs: state.targetMs, pageCount: state.pageCount, windowCount: state.windowCount, title: state.title, kind: state.kind }, { status, headers: { 'Cache-Control': 'no-store' } });
}
...
export async function POST(request: Request) {
  ...
  const relative = 'components/desktop/0915/3/run-configured.mjs';
  ...
  const child = spawn(process.execPath, ['--experimental-strip-types', script, JSON.stringify(settings), JSON.stringify(state.ownership4 ?? {})], { stdio: ['ignore', 'pipe', 'pipe'] });
  state.child = child;
  ...
  child.stdout?.on('data', chunk => {
    ...
    for (const line of lines) {
      try {
        ...
        if (event.phase === 'ownership') { state.ownership4 = event.ownership; state.pageCount = event.ownership.pages.length; state.windowCount = new Set(event.ownership.pages.map((p: { windowId: number }) => p.windowId)).size; }
        ...
        if (event.phase === 'running') { state.message = '실행 중'; state.progress = event.index; state.lastAction = event.app; state.elapsedMs = event.elapsedMs; state.targetMs = event.targetMs; state.pageCount = event.pageCount; state.windowCount = event.windowCount; state.title = event.title; state.kind = event.kind; }
