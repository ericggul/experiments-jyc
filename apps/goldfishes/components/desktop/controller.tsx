...
export default function DesktopController() {
  ...
  useEffect(() => {
    ...
    async function poll() {
      try {
        ...
        const response = await fetch('/api/desktop', { cache: 'no-store', signal: abort.signal });
  ...
  async function act(action: 'start' | 'stop') {
    ...
    try {
      const response = await fetch('/api/desktop', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action, settings: snapshot }) });
      const next = await response.json();
