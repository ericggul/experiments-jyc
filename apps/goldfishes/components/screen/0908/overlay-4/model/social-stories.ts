...
/** Rebuild topology for a new grid while retaining state for stable story IDs. */
export function resizeSocialStorySystem(
  system: SocialStorySystem,
  columns: number,
  rows: number,
  now: number,
...
export function stepSocialStorySystem(
  ...
  let randomSeed = system.randomSeed;
  const elapsed = Math.min(0.5, (now - system.time) / 1000);
  const resolvedStates = system.states.map((state, index) => {
    // Local attention prolongs visibility; crowding consumes it faster.
    // A hard age limit prevents immortal popular keywords.
  ...
  for (const source of shuffled.sources) {
    ...
    const localAttention = (attention.get(source) ?? 0) / Math.max(0.001, elapsed);
