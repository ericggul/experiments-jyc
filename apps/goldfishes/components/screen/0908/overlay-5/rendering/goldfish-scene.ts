...
import { APPROACH_PADDING } from "../model/approach-events";
...
const APPROACH_RING_VISUAL_PADDING = APPROACH_PADDING;
...
export class GoldfishScene {
  ...
  private ringRenderer: THREE.WebGLRenderer | undefined;
  ...
  private readonly approachRingMaterials: THREE.MeshBasicMaterial[] = [];
  private readonly approachRingMeshes: THREE.InstancedMesh[] = [];
  ...
  constructor(canvas: HTMLCanvasElement, private readonly traceCanvas: HTMLCanvasElement, private readonly ringCanvas: HTMLCanvasElement, count = 72, private readonly fishScale = 1) {
   ...
   */
  private ensureApproachRingRenderer() {
    if (this.ringRenderer) return this.ringRenderer;

    const renderer = new THREE.WebGLRenderer({ alpha: true, antialias: true, canvas: this.ringCanvas, powerPreference: "high-performance" });
    renderer.setPixelRatio(RING_RENDER_PIXEL_RATIO);
    renderer.outputColorSpace = THREE.SRGBColorSpace;
    renderer.setClearColor(0x000000, 0);
    renderer.setSize(this.width, this.height, false);

    const geometry = new THREE.RingGeometry(0.978, 1, 192);
    ...
    for (let bin = 0; bin < APPROACH_RING_ALPHA_BINS; bin += 1) {
      ...
      const mesh = new THREE.InstancedMesh(geometry, material, MAX_APPROACH_RING_STATES);
      mesh.count = 0;
      mesh.frustumCulled = false;
      mesh.instanceMatrix.setUsage(THREE.DynamicDrawUsage);
      this.approachRingMaterials.push(material);
      this.approachRingMeshes.push(mesh);
  ...
  private drawApproachRings(agents: readonly GoldfishAgent[], count: number, relations: TargetRelations, deltaSeconds: number) {
    ...
    const ringRenderer = this.ensureApproachRingRenderer();
    ...
    ringRenderer.render(this.ringScene, this.camera);
  }
