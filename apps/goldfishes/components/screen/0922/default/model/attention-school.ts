...
export class AttentionSchool {
  ...
  private readonly nearbyTargetBuffer: Target[] = [];
  ...
  updateTargets(system: SocialStorySystem) {
    ...
    system.states.forEach((state, index) => {
      ...
      this.targets.set(index, {
        index,
        x: this.relationState.centers[index * 2]!,
        y: this.relationState.centers[index * 2 + 1]!,
        radius: this.layout.iconSize * state.bubbleScale / 2,
        born: state.availableAt,
