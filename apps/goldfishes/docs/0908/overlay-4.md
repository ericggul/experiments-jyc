...
The `eyes` surface maps the 75 existing photographic human-eye collage assets in sequence across story nodes. Each image is cropped to its embedded eye circle, so active story circles fill with distinct eyes while the existing appearance timing, per-occurrence scale, rings, propagation and fish-target geometry remain unchanged. After 75 nodes the sequence repeats.
...
The `face` surface now exposes exactly two internal types: `politician`, preserving the existing 60-image set unchanged, and `big tech`, a separate 80-person 2026 technology-power set. The latter spans platform and infrastructure executives, AI lab leaders and researchers, founders, investors and other people with material institutional influence; inclusion is not praise, endorsement, or a claim that every person holds equivalent power. Both types receive the existing colour-overlay treatment. For `big tech`, colour assignment follows a neutral deterministic sequence and must not be read as an allegation about an individual's political ideology. The repeated, rapidly propagating face field supplies the critical relation; the interface adds no accusatory caption or unsupported intent claim.
