...
export default function SpliceIndexPage() {
  return (
    <main className="min-h-screen bg-[#f2f1ed] p-8 text-[#242421] sm:p-14">
      <Link target="_blank" rel="noopener noreferrer" href="/" className="text-sm underline underline-offset-4">SCC</Link>
      ...
      <nav aria-label="Splice experiments" className="grid gap-8">
        {spliceExperiments.map((experiment) => (
          <Link target="_blank" rel="noopener noreferrer" key={experiment.slug} href={`/splice/${experiment.slug}`} className="block w-fit">
